package com.project.util.RSAUtil;
...
public class RSATester<main> {
	...
	public void tests() throws Exception {
		/*
		 * String data =
		 * "hNYdT0/8wxljGvLQVPYUDeoWS217rmB5msKOljb6OBsOhdL1Domxt6Sy3BqdGqS3StGJuZuQ9wEFzGdyoTQ7IRiC0gRrkLuxvCUq8FANt1JpJCBqvzLdQD/ygCjkJMWWkMU4EFUW3xYkAsidqM8Zynhpk+mdja3789Ng4s68xJI="
		 * ;
		 * 
		 * String privateKey =
	...
	static void test() throws Exception {
		...
		String json = "{\"actname\":\"省内400M\",\"phoneno\":\"13955118215\"}";
		String source = json;
		System.out.println("\r加密前文字：\r\n" + source);
		byte[] data = source.getBytes();
		byte[] encodedData = RSAUtils.encryptByPublicKey(data, publicKey);
		System.out.println("加密后文字：\r\n" + new String(encodedData));
		String mstr=Base64Utils.encode(encodedData);
		
		System.out.println("----------------:base64处理："
				+mstr);
		System.out.println("----------------:base64处理："
				+new String( Base64Utils.decode(mstr)));
	...
	static void testJavaRsa() {
		...
		System.out.println("解密数据："
				+ RSAUtils.decryptDataOnJava(data, PRIVATEKEY));
	}
